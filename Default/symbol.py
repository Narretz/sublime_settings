...
def navigate_to_symbol(view, symbol, locations):
    def select_entry(window, locations, idx, orig_view, orig_sel):
        if idx >= 0:
            # if the hit is in the current view, change the selection
            if orig_view and orig_view.id() == window.active_view().id():
                fname, display_fname, rowcol = locations[idx]
                row, col = rowcol
                orig_view.sel().clear()
                orig_view.sel().add(sublime.Region(orig_view.text_point(row-1, col-1)))
            else:
    ...
    def highlight_entry(window, locations, idx):
        ...
        # highlight the hit if it is in the active view
        if active_view.file_name() == fname:
            active_view.sel().clear();
            active_view.sel().add(sublime.Region(active_view.text_point(row - 1, col - 1)))
            active_view.show(active_view.text_point(row, col))
        else:
            window.open_file(
                fname + ":" + str(row) + ":" + str(col),
